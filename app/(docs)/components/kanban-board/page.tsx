...
import {
  ...
  type KanbanColumn,
  type KanbanTag,
} from "@/registry/wise-ui/components/kanban-board"
...
const initialColumns: KanbanColumn<Task>[] = [
  {
    ...
    title: "Backlog",
    items: [
      { id: "t1", title: "Research competitor features", tagId: "design" },
      { id: "t2", title: "Write migration script", tagId: "dev" },
    ],
  },
  {
    id: "todo",
    title: "To Do",
    items: [
      { id: "t3", title: "Design card component", tagId: "design" },
      { id: "t4", title: "Fix dropdown z-index", tagId: "bug" },
      { id: "t5", title: "Write API docs", tagId: "docs" },
    ],
  },
  {
    id: "in-progress",
    title: "In Progress",
    items: [
      { id: "t6", title: "Build sidebar nav", tagId: "dev" },
      { id: "t7", title: "Update color tokens", tagId: "design" },
    ],
  },
  {
    id: "done",
    title: "Done",
    items: [
      { id: "t8", title: "Set up project", tagId: "dev" },
      { id: "t9", title: "Create theme provider", tagId: "dev" },
      { id: "t10", title: "Initial wireframes", tagId: "design" },
    ],
  },
]
...
const usageCode = `import { KanbanBoard, type KanbanColumn, type KanbanTag } from "@/components/ui/kanban-board"

interface Task { id: string; title: string; tagId?: string }

const tags: KanbanTag[] = [
  { id: "design", label: "Design", color: "bg-violet-500/15 text-violet-600" },
  { id: "dev", label: "Dev", color: "bg-sky-500/15 text-sky-600" },
  { id: "bug", label: "Bug", color: "bg-red-500/15 text-red-600" },
]

const initial: KanbanColumn<Task>[] = [
  { id: "todo", title: "To Do", items: [{ id: "1", title: "Design cards", tagId: "design" }] },
  { id: "doing", title: "Doing", items: [{ id: "2", title: "Build nav", tagId: "dev" }] },
...
export default function Example() {
  ...
  return (
    <KanbanBoard
      ...
      tags={tags}
      getItemTagId={(task) => task.tagId}
      renderItem={(task) => (
        <div className="rounded-lg border border-border bg-card p-3">
          <p className="text-sm text-foreground">{task.title}</p>
        </div>
      )}
      onAddItem={(colId, title, tagId) => {
        setColumns((cols) =>
          cols.map((c) =>
            c.id === colId
              ? { ...c, items: [...c.items, { id: crypto.randomUUID(), title, tagId }] }
              : c
          )
        )
      }}
...
# Create components/ui/kanban-board.tsx
...
const kanbanBoardProps = [
  {
    ...
    type: "KanbanColumn<T>[]",
    default: "-",
    description: "Array of columns, each with `id`, `title`, and `items`.",
  },
  {
    name: "onColumnsChange",
    type: "(columns) => void",
    default: "-",
    description: "Called when items are reordered or moved between columns.",
  },
  {
    name: "renderItem",
    type: "(item: T, columnId: string) => ReactNode",
    default: "-",
    description: "Render function for each item card.",
  },
  {
    name: "onAddItem",
    type: "(columnId: string, title: string, tagId?: string) => void",
    default: "-",
    description: "Called when a new item is added via the inline form. Shows add button if provided.",
  },
  {
    name: "getItemTagId",
    type: "(item: T) => string | undefined",
    default: "-",
    description: "Extract the tag id from an item. Required for the tag filter bar to appear.",
  },
  {
    name: "tags",
    type: "KanbanTag[]",
    default: "-",
    description: "Available tags shown in the add-item form. Each tag has `id`, `label`, and optional `color` class.",
  },
  {
    name: "collapsible",
    type: "boolean",
    default: "true",
    description: "Whether columns can be collapsed to a narrow strip.",
  },
  {
    name: "minColumnWidth",
    type: "number",
    default: "200",
    description: "Minimum width of each expanded column. Columns flex to fill available space.",
  },
  {
    name: "collapsedColumnWidth",
    type: "number",
    default: "44",
    description: "Width of each collapsed column in pixels.",
  },
  {
    name: "className",
    type: "string",
    default: "-",
    description: "Class applied to the board container.",
  },
  {
    name: "columnClassName",
    type: "string",
    default: "-",
    description: "Class applied to each column.",
  },
]
...
function KanbanBoardDemo() {
  ...
  function handleAddItem(columnId: string, title: string, tagId?: string) {
    setColumns((cols) =>
      cols.map((col) =>
        col.id === columnId
          ? {
          ...
          <div className="rounded-lg border border-border bg-card p-3 transition-colors">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                {task.tagId && tagMap[task.tagId] && (
                  <span
                    className={`mt-1.5 inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${tagMap[task.tagId].color}`}
                  >
                    {tagMap[task.tagId].label}
                  </span>
                )}
              </div>
            </div>
...
export default function KanbanBoardPage() {
  return (
    <ComponentPage
      name="Kanban Board"
      description="Multi-column drag-and-drop board with cross-column transfers, collapsible columns, tag support, and inline add-item forms."
      code={usageCode}
      cliCommand="npx shadcn@latest add https://wise-ui.com/r/kanban-board.json"
